...
read_data_sets = contrib.learn.datasets.mnist.read_data_sets
...
def parser_():
    ...
    parser.add_argument('--workdir', type=str, default=None,
                        help='Directory for storing training graph. Default '
                        'does not save.')
    return parser
...
def main(argv=None):
    ...
    pargs = parser_()
    FLAGS, _ = pargs.parse_known_args()
    ...
    workdir = FLAGS.workdir if FLAGS.workdir else None
    if workdir:
    ...
    train_loops = 2000  # 20000

    # NOTE: Illustrate how to manage Tensorflow memory.
    ...
    with tf.Session() as sess:
        ...
        if workdir:
            path_ = os.path.join(workdir, 'mnist-model')
            saver.save(sess, path_, write_meta_graph=False)
